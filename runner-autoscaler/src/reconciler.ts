...
export class Reconciler {
  ...
  private async handleQueuedJob(job: QueuedJob, scope: Scope): Promise<void> {
    ...
    const memGate = evaluateMemoryGate(memSnap, {
      memoryFloorBytes: config.pool.memoryFloorMiB * 1024 * 1024,
      runnerEstimatedBytes: config.pool.runnerEstimatedMiB * 1024 * 1024,
    });
    if (!memGate.allow) {
      log.warn(
        {
          ...
          floor_mib: config.pool.memoryFloorMiB,
          reason: memGate.reason,
        },
        "reconcile: crime=host-memory-floor; cannot spawn for stranded job",
      );
      return;
    }
